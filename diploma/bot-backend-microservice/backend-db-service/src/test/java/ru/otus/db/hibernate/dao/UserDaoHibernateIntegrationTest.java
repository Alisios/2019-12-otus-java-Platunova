...
@DisplayName("Тесты проверяют: ")
 public class UserDaoHibernateIntegrationTest {
    ...
    @DisplayName("проверку если User уже присуствует в БД")
    @ParameterizedTest
    @MethodSource("generateUsers")
    void checkIfUserIsAlreadyInTheTable(User user){
        long id = dbServiceUser.saveUser(user);
        User userIncorrect1 = new User(202812832, new ConcertModel("TWENTY ØNE PILØTS",
                "12 Июльвс 19:00",
        ...
        User userIncorrect2 = new User(202812830, new ConcertModel("beatles",
                "12 Июльвс 19:00",
                "ВТБ Арена – Центральный стадион «Динамо» имени Льва Яшина",
                "https://msk.kassir.ru/koncert/twenty-one-pilots#199390"),
                new GregorianCalendar(2020, 6,5).getTime());
        User userIncorrect3 = new User(2028128334, new ConcertModel("beatles",
                "12 Июльвс 19:00",
                "ВТБ Арена – Центральный стадион «Динамо» имени Льва Яшина",
                "https://msk.kassir.ru/koncert/twenty-one-pilots#199390"),
                new GregorianCalendar(2020, 6,5).getTime());
        assertTrue(dbServiceUser.findByChatIdAndArtist(user).isPresent());
        assertFalse(dbServiceUser.findByChatIdAndArtist(userIncorrect1).isPresent());
        assertFalse(dbServiceUser.findByChatIdAndArtist(userIncorrect2).isPresent());
        assertFalse(dbServiceUser.findByChatIdAndArtist(userIncorrect3).isPresent());
        assertEquals(dbServiceUser.saveUser(user), id);
        assertNotEquals(dbServiceUser.saveUser(userIncorrect1), id);
        assertNotEquals(dbServiceUser.saveUser(userIncorrect2), id);
        assertNotEquals(dbServiceUser.saveUser(userIncorrect3), id);

       logger.info("Проверка Postres {}", dbServiceUser.getAllUsers().toString());
    }
...
//    }
    ...
    private static Stream<Arguments> generateUsers() {
        return Stream.of(
                Arguments.of((new User(202812830, new ConcertModel("Green Day",
                                "24 Майвс 19:00",
