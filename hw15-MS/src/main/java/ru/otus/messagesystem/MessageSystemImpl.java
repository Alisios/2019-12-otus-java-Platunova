...
public final class MessageSystemImpl implements MessageSystem {
    ...
    private static final int MSG_HANDLER_THREAD_LIMIT = 4;
    ...
    @Override
    public boolean newMessage(Message msg) {//передавать тип сообщения который определяет тип очереди куда пихать
        if (runFlag.get()) {
    ...
    private void processMessages() {
        ...
        while (runFlag.get() || !messageQueue.isEmpty()) {
            //вот тут условие в зависимости от типа очередей - >разные хенлеры и разный пул потоков
            try {
                ...
                if (msg == Message.VOID_MESSAGE) {
                    ...
                    if (clientTo == null) {
                        ...
                        logger.info("handler! in process");
